#!/usr/bin/env python3
"""
RSI(5) Under 10MA Over 200MA Strategy Implementation using vectorbt

Simple and effective entry signal strategy:
- Universe: S&P 500 stocks  
- Entry Signal: Price > 200-day MA AND Price < 10-day MA AND RSI(5) < 15
- Exit Signal: Price > 10-day moving average
- Position Sizing: Max 10 open positions, 10% capital per position
- If >10 signals, choose the 10 stocks with the lowest RSI(5)
...
class RSI5_Under10MA_Over200MA_Strategy:
    """RSI(5) Under 10MA Over 200MA Strategy using vectorbt for backtesting."""
    
    def __init__(self, config: StrategyConfig = None):
        """Initialize the strategy with configuration."""
        self.config = config or DEFAULT_CONFIG
        # Override RSI settings for this strategy
        self.config.rsi_period = 5
        self.config.rsi_entry_threshold = 15.0
        self.client = None
        self.close_df = None
        self.high_df = None
        self.low_df = None
    ...
    def generate_signals(self, indicators: Dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            # Simple entry condition: RSI(5) < 15 AND Price > 200-day MA AND Price < 10-day MA
    ...
    def apply_position_limit(self, basic_entries: pd.DataFrame, rsi_values: pd.DataFrame) -> pd.DataFrame:
        """Apply max 10 positions rule by selecting stocks with lowest RSI(5)."""
        try:
    ...
    def plot_results(self, save_path: str = None):
        ...
        try:
            ...
            fig = portfolio_value.vbt.plot(
                title="RSI(5) Under 10MA Over 200MA Strategy - Equity Curve"
            )
            ...
            try:
                ...
                if hasattr(drawdowns, 'plot'):
                    drawdowns_fig = drawdowns.plot(
                        title="RSI(5) Under 10MA Over 200MA Strategy - Drawdowns"
                    )
    ...
    def run_strategy(self) -> Dict:
        """Run the complete RSI(5) Under 10MA Over 200MA strategy."""
        try:
            logger.info("Starting RSI(5) Under 10MA Over 200MA Strategy...")
            ...
            if results:
                print("\n" + "="*60)
                print("RSI(5) UNDER 10MA OVER 200MA STRATEGY RESULTS")
                print("="*60)
...
def main():
    """Main function to run the RSI(5) Under 10MA Over 200MA strategy."""
    ...
    strategy = RSI5_Under10MA_Over200MA_Strategy(config)
    results = strategy.run_strategy()
    
    # Optional: Generate plots
    if strategy.portfolio and results:
        strategy.plot_results("rsi5_under10ma_over200ma_strategy_results")
