#!/usr/bin/env python3
"""
RSI(3) Combined Signal Strategy Implementation using vectorbt

Combined entry signal strategy:
...
class RSI3CombinedStrategy:
    """RSI(3) Combined Signal Strategy using vectorbt for backtesting."""
    ...
    def generate_signals(self, indicators: Dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            # Combined entry condition: RSI(3) < 10 AND Price > 200-day MA AND Price < 10-day MA
            basic_entries = (rsi_aligned < self.config.rsi_entry_threshold) & (close_aligned > ma_200_aligned) & (close_aligned < ma_10_aligned)
    ...
    def plot_results(self, save_path: str = None):
        ...
        try:
            ...
            fig = portfolio_value.vbt.plot(
                title="RSI(3) Combined Signal Strategy - Equity Curve"
            )
            ...
            try:
                ...
                if hasattr(drawdowns, 'plot'):
                    drawdowns_fig = drawdowns.plot(
                        title="RSI(3) Combined Signal Strategy - Drawdowns"
                    )
    ...
    def display_trade_analysis(self, analysis: Dict):
        ...
        # Best trades - commented out to remove from console output
        # print(f"\n{'-'*40}")
        # print(f"TOP {len(analysis['best_trades'])} BEST TRADES")
        # print(f"{'-'*40}")
        # best_cols = ['column', 'entry_timestamp', 'exit_timestamp', 'return_pct', 'duration_days', 'size']
        # if not analysis['best_trades'].empty:
        #     for idx, trade in analysis['best_trades'].iterrows():
        #         print(f"Symbol: {trade.get('symbol', 'N/A')}")
        #         print(f"  Entry: {trade.get('entry_timestamp', 'N/A')}")
        #         print(f"  Exit: {trade.get('exit_timestamp', 'N/A')}")
        #         print(f"  Return: {trade.get('return_pct', 0):.2f}%")
        #         print(f"  Duration: {trade.get('duration_days', 0):.0f} days")
        #         print(f"  Position Size: ${trade.get('size', 0):,.2f}")
        #         print(f"  PnL: ${trade.get('pnl', 0):,.2f}")
        #         print()
        
        # Worst trades - commented out to remove from console output
        # print(f"{'-'*40}")
        # print(f"TOP {len(analysis['worst_trades'])} WORST TRADES")
        # print(f"{'-'*40}")
        # if not analysis['worst_trades'].empty:
        #     for idx, trade in analysis['worst_trades'].iterrows():
        #         print(f"Symbol: {trade.get('symbol', 'N/A')}")
        #         print(f"  Entry: {trade.get('entry_timestamp', 'N/A')}")
        #         print(f"  Exit: {trade.get('exit_timestamp', 'N/A')}")
        #         print(f"  Return: {trade.get('return_pct', 0):.2f}%")
    ...
    def run_strategy(self) -> Dict:
        """Run the complete RSI(3) combined signal strategy."""
        try:
            logger.info("Starting RSI(3) Combined Signal Strategy...")
            ...
            if results:
                print("\n" + "="*60)
                print("RSI(3) COMBINED SIGNAL STRATEGY RESULTS")
                print("="*60)
...
def main():
    """Main function to run the RSI(3) combined signal strategy."""
    ...
    if strategy.portfolio and results:
        strategy.plot_results("rsi3_combined_strategy_results")
